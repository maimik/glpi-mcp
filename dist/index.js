...
if (!GLPI_API_URL || !GLPI_APP_TOKEN || (!GLPI_USER_TOKEN && !GLPI_OAUTH_TOKEN)) {
    console.error('Error: Missing required environment variables. Needs GLPI_API_URL, GLPI_APP_TOKEN, and either GLPI_USER_TOKEN or GLPI_OAUTH_TOKEN.');
    process.exit(1);
}
...
server.tool('glpi_list_items', listItemsSchema, async ({ itemType, range, sort, order, is_deleted, searchText }) => {
    try {
        const params = {};
        if (range)
            params.range = range;
...
server.tool('glpi_get_item', getItemSchema, async ({ itemType, id, expand_dropdowns, with_devices, with_disks, with_softwares, with_connections, get_hateoas, }) => {
    try {
        const params = {};
        if (expand_dropdowns)
            params.expand_dropdowns = true;
        if (with_devices)
            params.with_devices = true;
        if (with_disks)
            params.with_disks = true;
        if (with_softwares)
            params.with_softwares = true;
        if (with_connections)
            params.with_connections = true;
        if (get_hateoas)
            params.get_hateoas = true;
...
server.tool('glpi_create_item', createItemSchema, async ({ itemType, input }) => {
    try {
        ...
        const result = await glpi.createItem(itemType, parsedInput);
        return {
...
server.tool('glpi_update_item', updateItemSchema, async ({ itemType, id, input }) => {
    try {
        ...
        const result = await glpi.updateItem(itemType, id, parsedInput);
        return {
            content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
        };
    }
...
function getPotentialEndpoints(itemType) {
...
async function findPluginRecord(itemType, itemId, endpointOverride) {
    ...
    for (const endpoint of endpoints) {
        try {
            ...
            const results = (await glpi.listItems(endpoint, params));
...
server.tool('glpi_get_cia_fields', getCiaFieldsSchema, async ({ itemType, id, endpoint_override }) => {
    try {
...
// @ts-expect-error — zod + MCP SDK type instantiation too deep for TS
server.tool('glpi_update_cia_fields', updateCiaFieldsSchema, async ({ itemType, id, confidentialityfield, integrityfield, availabilityfield, data_sensitivity, other_fields, endpoint_override, }) => {
    const input = {};
    if (confidentialityfield !== undefined)
        input.confidentialityfield = confidentialityfield;
    if (integrityfield !== undefined)
        input.integrityfield = integrityfield;
    if (availabilityfield !== undefined)
        input.availabilityfield = availabilityfield;
    // Backward compatibility
    if (data_sensitivity && !input.confidentialityfield) {
        const num = parseInt(data_sensitivity);
        if (!isNaN(num))
            input.confidentialityfield = num;
    }
    if (other_fields) {
        try {
    ...
    try {
        ...
        if (found) {
            const result = await glpi.updateItem(found.endpoint, found.record.id, input);
            return {
                content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
            };
        }
...
server.tool('glpi_update_cia_fields_batch', updateCiaFieldsBatchSchema, async ({ itemType, ids, confidentialityfield, integrityfield, availabilityfield, other_fields, endpoint_override, }) => {
    try {
        const idList = ids
            .split(',')
            .map((s) => s.trim())
        ...
        for (const id of idList) {
            try {
                ...
                let res;
                if (found) {
                    res = await glpi.updateItem(found.endpoint, found.record.id, input);
                }
                else {
                    const endpoints = endpoint_override
                        ? [endpoint_override]
                    ...
                    if (!created)
                        throw new Error('Could not create record (checked all endpoints)');
                }
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({ success_count: results.length, error_count: errors.length, results, errors }, null, 2),
                },
            ],
        };
    }
